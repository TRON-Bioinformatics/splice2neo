test_that("liftover_junc_id works on toy example data", {
  ...
  expect_true(all(c("junc_id_lifted_unique", "junc_id_lifted") %in% names(junc_df_lifted)))
...
test_that("liftover_junc_id works with non-unique mappings", {
  ...
  expect_equal(nrow(junc_df_lifted), nrow(junc_df))
  expect_false(all(junc_df_lifted$junc_id_lifted_unique))
  expect_true(all(junc_df_lifted$junc_id_lifted_unique[1:3]))
