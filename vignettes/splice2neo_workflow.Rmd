...
We can retrieve the canonical splice junctions between all adjacent exons from a list of transcripts with `canonical_junctions()`. Here,  we use only the first ten transcripts.

```{r}
ref_junc <- canonical_junctions(transcripts[1:10])
...
The output of the tool [LeafCutter](https://github.com/davidaknowles/leafcutter) can be transformed using `leafcutter_transform()`: 
...
The output of the tool [SplAdder](https://github.com/ratschlab/spladder) can be transformed using `spladder_transform()`: 
...
Currently, the transformation of predictions by [SpliceAI](https://github.com/Illumina/SpliceAI), [MMSplice](https://github.com/gagneurlab/MMSplice_MTSplice) and [Pangolin](https://github.com/tkzeng/Pangolin) are supported.  
