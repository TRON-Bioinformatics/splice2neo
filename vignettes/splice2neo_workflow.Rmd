...
In this vignette, we use use some toy example data to demonstrate how splice2neo can be used to analyze splice-junctions which are derived from mutation predictions as tumor-specific neoantigen candidates. 
...
We can read the effect predictions by MMSplice with `parse_spliceai()`
...
To exclude junctions expressed in healthy tissues, we here want to filter predicted junctions for non-canoncal splice junctions. We can use for instance exon-exon junctions from GENCODE and junctions found in healthy tisssue samples. Splice2neo also provides the function `bed_to_junc()` which allows to transform a bed file into a list of splice junctions. 
...
Altered transcript from predicted splice junctions can be re-quantified in RNA-seq in a targeted manner with EasyQuant (available at  [https://github.com/TRON-Bioinformatics/easyquant](https://github.com/TRON-Bioinformatics/easyquant)). EasyQuant mapps all RNA-seq reads to to a custom reference build from the context sequence around splice junctions. We can generate the input for EasyQuant using the splice2neo function `transform_for_requant()`.  
...
Given a `data.frame` of splice junctions (with a colum `junc_id`), the function `add_tx()` annotates all possible transcripts which overlap
