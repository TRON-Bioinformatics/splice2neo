...
This R package splice2neo is not yet on [CRAN](https://CRAN.R-project.org) or
...
A database of transcripts is required for transcript annotation with splice2neo. Here, we show how to build the database from GENCODE annotation but users can build these databases also from other resources.  
...
Splice2neo transforms splice events identified in RNA-seq into a standardized junction format. Currently, the transformation of results by LeafCutter and SplAdder are supported. 

#### LeafCutter 

LeafCutter results can be transformed using `leafcutter_transform`: 
...
SplAdder results can be transformed using `leafcutter_transform`: 
...
Get a dataframe of all splice junctions identified by RNA-seq tools and information by which tool they were identified. 

```{r}
rna_junctions <-
  generate_combined_dataset(spladder_juncs = spladder, leafcutter_juncs = leafcutter)
...
Splice2neo transforms the predicted effect of mutations on splicing into unified splice junction format. Currently, the transformation of predictions by SpliceAI, MMSplice and Pangolin are supported.  
Here, we show how to annotate results from SpliceAI and MMSplice. The annotation of Pangolin results works analogue to SpliceAI results and is described in the README. 
...
Next, SpliceAi effects can be transformed into a standardized effect dataframe with `format_spliceai`. Here, the user can optionally provide a dataframe (`gene_table`) which relates gene symbols with GENCODE gene ids. If `gene_table` is provided,  the formatted dataframe contains the GENCODE gene ids. Importantly, this advised to better select relevant transcripts but it is not required. 
...
Effect predictions by MMSplice can be read with `parse_spliceai`
...
MMSplice can be annotated with all potential splice junctions using `annotate_mmsplice`
...
Junctions predicted from MMSplice results can contain duplicated altered transcripts for exon inclusion events. The function `unique_junc_mmsplice ` can be used to filter for unique altered transcripts by keeping the effect with the highest effect score. 
...
Splice junctions from SpliceAI and MMSplice can be combined into one dataframe using `combine_mut_junc`. This dataframe will contain unique altered transcripts (defined by `mut_id`, `junc_id`, `tx_id`) in rows. Tool specific information are defined in columns starting with the respective tool name.    
...
Users may want to filter predicted junctions for non-canoncal splice junctions. Here, users could for instance use exon-exon junctions from GENCODE and junctions found in healthy tisssue samples. Splice2neo provides the function `bed_to_junc` which allows to transform a bed file into a list of splice junctions. Users need to generate the bed file with canonical junctions by themselves.  
...
Splice2neo can annotate the resulting transcript sequence for each altered transcript with `add_context_seq`. For instance, this is relevant for the targeted re-quantification of splice junctions with EasyQuant (see below). 
...
Splice2neo can annotate the resulting peptide sequence for each altered transcript with `add_peptide`. This is relevant to analyze which altered transcript could qualify as neoantigens. Users can define by the parameter `flanking_size` the number of wild-type amino acids flanking the junction or novel sequence caused by the splice junction to the left and to the right. Frame shift sequences are flanked by wild-type amino acids to the left and are annotated until the first stop codon.
...
Relevant intron retention events are particular challenging to predict. Splice2neo provides the function `exon_in_intron` to annotate whether the exon of another transcript is in the genomic region of a predicted retained introns. Users may not want to consider them in the downstream analysis as it is difficult to differentiate if the read support from RNA-seq relates to the retained intron or to the exon of the other transcript.  
...
Altered transcript from predicted splice junctions can be re-quantified in RNA-seq in a targeted manner with EasyQuant (available at  [https://github.com/TRON-Bioinformatics/easyquant](https://github.com/TRON-Bioinformatics/easyquant)). Users can generate the input for EasyQuant using the splice2neo function `transform_for_requant`.  
EasyQuant should be run in interval mode for the re-quantification of splice junction and it is advised to set the BP_distannce parameter to 10. For more information please refer to the EasyQuant [README](https://github.com/TRON-Bioinformatics/easyquant). 
...
# DO RE-QUANTIFICATION WITH EASYQUANT using easyquant_input
# https://github.com/TRON-Bioinformatics/easyquant

```

After running EasyQuant, splice2neo supports import of EasyQuant results. 
...
The user may be interested in different result columns depending on the type of splice
event. In case of alternative splice sites or exon skipping events, users may want use  the number of junction reads that map on the splice junction
(`junc_interval_start`) and/or the number of spanning reads that frame the splice junction (`span_interval_start`). 
...
In case of intron retentions, users may want to use the following columns:  
...
### Prediction of mutation-assoicated splice targets  

The upstream analysis identified mutation-retrieved splice junctions supported by RNA-seq. We have developed a stringent detection rule to predict tumor-specific targets from mutation-associated splice targets based on the mutation effect and the RNA-seq support (for further information please check our pre-print: https://doi.org/10.1101/2023.06.27.546494). Currently, we suggest to focus on events from alternative splice sites and exon skipping and define such splice junctions as targets that have a |splice effect score| >= 0.35 and are found by LeafCutter/SplAdder or have at least 3 junction reads.  
...
Please note that `targets` can contain several altered transcripts per junctions which may lead to several neoantigen candidate sequences per target splice junction. 
...
Users may have a list of splice junctions and want to know all potential affected transcripts. 

Given a dataframe of splice junctions, the `add_tx` function annotates all possible transcripts which overlap
